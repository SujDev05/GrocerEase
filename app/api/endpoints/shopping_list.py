...
from app.api.crud.shopping_list import (
...
def add_shopping_list(shopping_data: ShoppingListCreate, db: Session = Depends(get_db)):
    """
    Create a new shopping list entry.
    """
    return create_shopping_list(db, shopping_data)
...
def update_shopping_list_items(
    user_id: int, shopping_data: ShoppingListCreate, db: Session = Depends(get_db)
):
    shopping_list = update_shopping_list(db, user_id, shopping_data)
    if not shopping_list:
        raise HTTPException(status_code=404, detail="Shopping list not found")
    return shopping_list
...
def remove_shopping_list(user_id: int, db: Session = Depends(get_db)):
    success = delete_shopping_list(db, user_id)
    if not success:
        raise HTTPException(status_code=404, detail="Shopping list not found")
